...
TIrrSocket::TIrrSocket(int port)
...
TIrrSocket::TIrrSocket(int port, int sendTimeout, int recvTimeout)
  : interruptableChildren_(true),
    port_(port),
    serverSocket_(THRIFT_INVALID_SOCKET),
    acceptBacklog_(DEFAULT_BACKLOG),
...
TIrrSocket::TIrrSocket(const string& address, int port)
  : interruptableChildren_(true),
    port_(port),
    address_(address),
    serverSocket_(THRIFT_INVALID_SOCKET),
...
TIrrSocket::TIrrSocket(const string& path)
  : interruptableChildren_(true),
    port_(0),
    path_(path),
    serverSocket_(THRIFT_INVALID_SOCKET),
...
TIrrSocket::~TIrrSocket() {
  close();
}

void TIrrSocket::setSendTimeout(int sendTimeout) {
  sendTimeout_ = sendTimeout;
}

void TIrrSocket::setRecvTimeout(int recvTimeout) {
  recvTimeout_ = recvTimeout;
}

void TIrrSocket::setAcceptTimeout(int accTimeout) {
  accTimeout_ = accTimeout;
}

void TIrrSocket::setAcceptBacklog(int accBacklog) {
  acceptBacklog_ = accBacklog;
}

void TIrrSocket::setRetryLimit(int retryLimit) {
  retryLimit_ = retryLimit;
}

void TIrrSocket::setRetryDelay(int retryDelay) {
  retryDelay_ = retryDelay;
}

void TIrrSocket::setTcpSendBuffer(int tcpSendBuffer) {
  tcpSendBuffer_ = tcpSendBuffer;
}

void TIrrSocket::setTcpRecvBuffer(int tcpRecvBuffer) {
  tcpRecvBuffer_ = tcpRecvBuffer;
}

void TIrrSocket::setInterruptableChildren(bool enable) {
  if (listening_) {
...
void TIrrSocket::listen() {
  ...
  if (-1 == THRIFT_SOCKETPAIR(AF_LOCAL, SOCK_STREAM, 0, sv)) {
    GlobalOutput.perror("TIrrSocket::listen() socketpair() interrupt", THRIFT_GET_SOCKET_ERROR);
    interruptSockWriter_ = THRIFT_INVALID_SOCKET;
    interruptSockReader_ = THRIFT_INVALID_SOCKET;
  } else {
  ...
  if (-1 == THRIFT_SOCKETPAIR(AF_LOCAL, SOCK_STREAM, 0, sv)) {
    GlobalOutput.perror("TIrrSocket::listen() socketpair() childInterrupt",
  ...
  if (serverSocket_ == THRIFT_INVALID_SOCKET) {
    ...
    GlobalOutput.perror("TIrrSocket::listen() socket() ", errno_copy);
    close();
    throw TTransportException(TTransportException::NOT_OPEN,
...
// want to force servers to be an admin.
#ifndef _WIN32
    int errno_copy = THRIFT_GET_SOCKET_ERROR;
    GlobalOutput.perror("TIrrSocket::listen() setsockopt() THRIFT_NO_SOCKET_CACHING ",
                        errno_copy);
    close();
    throw TTransportException(TTransportException::NOT_OPEN,
  ...
  if (tcpSendBuffer_ > 0) {
    if (-1 == setsockopt(serverSocket_,
      ...
      int errno_copy = THRIFT_GET_SOCKET_ERROR;
      GlobalOutput.perror("TIrrSocket::listen() setsockopt() SO_SNDBUF ", errno_copy);
      close();
      throw TTransportException(TTransportException::NOT_OPEN,
  ...
  if (tcpRecvBuffer_ > 0) {
    if (-1 == setsockopt(serverSocket_,
      ...
      int errno_copy = THRIFT_GET_SOCKET_ERROR;
      GlobalOutput.perror("TIrrSocket::listen() setsockopt() SO_RCVBUF ", errno_copy);
      close();
      throw TTransportException(TTransportException::NOT_OPEN,
...
// Defer accept
  ...
  if (path_.empty()) {
    if (-1 == setsockopt(serverSocket_, IPPROTO_TCP, TCP_DEFER_ACCEPT, &one, sizeof(one))) {
      int errno_copy = THRIFT_GET_SOCKET_ERROR;
      GlobalOutput.perror("TIrrSocket::listen() setsockopt() TCP_DEFER_ACCEPT ", errno_copy);
      close();
      throw TTransportException(TTransportException::NOT_OPEN,
  ...
  if (res->ai_family == AF_INET6 && path_.empty()) {
    ...
    if (-1 == setsockopt(serverSocket_,
                         ...
                         cast_sockopt(&zero),
                         sizeof(zero))) {
      GlobalOutput.perror("TIrrSocket::listen() IPV6_V6ONLY ", THRIFT_GET_SOCKET_ERROR);
    }
  }
  ...
  if (-1 == setsockopt(serverSocket_, SOL_SOCKET, SO_LINGER, cast_sockopt(&ling), sizeof(ling))) {
    int errno_copy = THRIFT_GET_SOCKET_ERROR;
    GlobalOutput.perror("TIrrSocket::listen() setsockopt() SO_LINGER ", errno_copy);
    close();
  ...
  if (path_.empty()) {
    ...
    if (-1
      ...
      int errno_copy = THRIFT_GET_SOCKET_ERROR;
      GlobalOutput.perror("TIrrSocket::listen() setsockopt() TCP_NODELAY ", errno_copy);
      close();
      throw TTransportException(TTransportException::NOT_OPEN,
  ...
  if (flags == -1) {
    int errno_copy = THRIFT_GET_SOCKET_ERROR;
    GlobalOutput.perror("TIrrSocket::listen() THRIFT_FCNTL() THRIFT_F_GETFL ", errno_copy);
    close();
    throw TTransportException(TTransportException::NOT_OPEN,
  ...
  if (-1 == THRIFT_FCNTL(serverSocket_, THRIFT_F_SETFL, flags | THRIFT_O_NONBLOCK)) {
    int errno_copy = THRIFT_GET_SOCKET_ERROR;
    GlobalOutput.perror("TIrrSocket::listen() THRIFT_FCNTL() THRIFT_O_NONBLOCK ", errno_copy);
    close();
    throw TTransportException(TTransportException::NOT_OPEN,
  ...
  if (!path_.empty()) {
    ...
    if (port_ == 0 && retries <= retryLimit_) {
      ...
      if (::getsockname(serverSocket_, reinterpret_cast<struct sockaddr*>(&sa), &len) < 0) {
        errno_copy = THRIFT_GET_SOCKET_ERROR;
        GlobalOutput.perror("TIrrSocket::getPort() getsockname() ", errno_copy);
      } else {
  ...
  if (retries > retryLimit_) {
    char errbuf[1024];
    if (!path_.empty()) {
      THRIFT_SNPRINTF(errbuf, sizeof(errbuf), "TIrrSocket::listen() PATH %s", path_.c_str());
    } else {
      THRIFT_SNPRINTF(errbuf, sizeof(errbuf), "TIrrSocket::listen() BIND %d", port_);
    }
    GlobalOutput(errbuf);
    close();
    throw TTransportException(TTransportException::NOT_OPEN,
  ...
  if (-1 == ::listen(serverSocket_, acceptBacklog_)) {
    errno_copy = THRIFT_GET_SOCKET_ERROR;
    GlobalOutput.perror("TIrrSocket::listen() listen() ", errno_copy);
    close();
...
int TIrrSocket::getPort() {
  return port_;
}

shared_ptr<TTransport> TIrrSocket::acceptImpl() {
  if (serverSocket_ == THRIFT_INVALID_SOCKET) {
    throw TTransportException(TTransportException::NOT_OPEN, "TIrrSocket not listening");
  }
  ...
  if (clientSocket == THRIFT_INVALID_SOCKET) {
    int errno_copy = THRIFT_GET_SOCKET_ERROR;
    GlobalOutput.perror("TIrrSocket::acceptImpl() ::accept() ", errno_copy);
  ...
  if (flags == -1) {
    ...
    GlobalOutput.perror("TIrrSocket::acceptImpl() THRIFT_FCNTL() THRIFT_F_GETFL ", errno_copy);
  ...
  if (-1 == THRIFT_FCNTL(clientSocket, THRIFT_F_SETFL, flags & ~THRIFT_O_NONBLOCK)) {
    ...
    ::THRIFT_CLOSESOCKET(clientSocket);
    GlobalOutput
        .perror("TIrrSocket::acceptImpl() THRIFT_FCNTL() THRIFT_F_SETFL ~THRIFT_O_NONBLOCK ",
                errno_copy);
    throw TTransportException(TTransportException::UNKNOWN,
...
shared_ptr<TSocket> TIrrSocket::createSocket(THRIFT_SOCKET clientSocket) {
...
void TIrrSocket::notify(THRIFT_SOCKET notifySocket) {
  if (notifySocket != THRIFT_INVALID_SOCKET) {
    int8_t byte = 0;
    if (-1 == send(notifySocket, cast_sockopt(&byte), sizeof(int8_t), 0)) {
      GlobalOutput.perror("TIrrSocket::notify() send() ", THRIFT_GET_SOCKET_ERROR);
    }
  }
}

void TIrrSocket::interrupt() {
...
void TIrrSocket::interruptChildren() {
  concurrency::Guard g(rwMutex_);
